...
--}
local _DB
local _DBNAME = C_AddOns.GetAddOnMetadata(_NAME, 'X-oUF-MovableFrames-SavedVariables')
local _LOCK
local _TITLE = C_AddOns.GetAddOnMetadata(_NAME, 'X-oUF-MovableFrames-Title')
local _DISPLAYNAMES = C_AddOns.GetAddOnMetadata(_NAME, 'X-oUF-MovableFrames-DisplayNames')
local _CATEGORY 
...
local getDisplayName = function(styleName, frameName)
	local db = _DISPLAYNAMES and _G[_DISPLAYNAMES]
	if (not db) then return end 

	if (frameName) then 
		return db[styleName] and db[styleName].frames and db[styleName].frames[frameName] or frameName
	else
		_G.print(db[styleName].header, type(db[styleName].header))
		return db[styleName] and db[styleName].header or styleName
	end
end
...
do
	local OnShow = function(self)
		return self.name:SetText(self.target.displayName or smartName(self.obj, self.header))
	end
	...
	getBackdrop = function(obj, isHeader)
		...
		backdrop:SetBackdropBorderColor(0, .4, .9) -- 0, .9, 0
		backdrop:SetBackdropColor(0, .4, .9, .6) -- 0, .9, 0
			...
			for style, styleData in next, _DB do
				if(style ~= '__INITIAL') then
					if(not data[numStyles]) then
						...
						if(numStyles == 1) then
							box:SetPoint('TOP', 0, -20) -- 0, -12
						else
							box:SetPoint('TOP', data[numStyles - 1], 'BOTTOM', 0, -24) -- 0, -16
						end
						box:SetPoint('LEFT')
						box:SetPoint('RIGHT', scroll, 0, 0)

						local title = box:CreateFontString(nil, 'ARTWORK', 'GameFontHighlight')
						title:SetPoint('BOTTOMLEFT', box, 'TOPLEFT', 8, 2) -- 8, 0
						box.title = title

						if(numStyles == 1) then
							local scaleTitle = box:CreateFontString(nil, nil, 'GameFontHighlight')
							scaleTitle:SetPoint('BOTTOMRIGHT', box, 'TOPRIGHT', -42, 0) -- -35, 0
							...
							yTitle:SetPoint('RIGHT', scaleTitle, 'LEFT', -5, 0) -- -5, 0
							yTitle:SetWidth(40)
							yTitle:SetText('Y')
							...
							xTitle:SetPoint('RIGHT', yTitle, 'LEFT', -5, 0) -- -5, 0
							xTitle:SetWidth(40)
							xTitle:SetText('X')
					...
					box.title:SetText(getDisplayName(style) or style) -- group title
					...
					for unit, points in next, styleData do
						if(not rows[numFrames]) then
							...
							delete:SetWidth(30) -- 16
							delete:SetHeight(30) -- 16
							delete:SetPoint('LEFT', row, 'RIGHT', -4, 0) -- 0, 0
						...
						row.unitLabel:SetText(getDisplayName(style, unit) or smartName(unit))
					...
					local height = (numFrames * 32) - 16 -- (numFrames * 24) - 8
...
local slashList = C_AddOns.GetAddOnMetadata(_NAME, 'X-oUF-MovableFrames-SlashCmdList'):gsub('%s+', '')
